...
interface MemberDetailContentProps {
  ...
  isAdmin: boolean;
  canEdit?: boolean;
  /** Đời tính động theo gốc đang chọn; nếu bỏ trống dùng person.generation. */
...
export default function MemberDetailContent({
  ...
  const generationValue = generation !== undefined ? generation : person.generation;
  const [isNoteExpanded, setIsNoteExpanded] = useState(false);
  // Read-only build: children/grand-children stats came from the editable
  // RelationshipManager (removed). Kept null so the stats card simply hides.
  const relStats = null as {
