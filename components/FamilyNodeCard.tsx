...
interface FamilyNodeCardProps {
  ...
  /** Vợ/chồng thứ 2 trở đi — đánh dấu "+" ở mép thẻ. */
...
export default function FamilyNodeCard({
  ...
  // Năm sinh – năm mất. Khuyết năm nào thì "?" chỗ đó; còn sống chỉ hiện năm sinh;
  // không biết năm nào cả thì bỏ hẳn dòng này.
  const { birth_year: birth, death_year: death } = person;
  let lifespan: string | null = null;
  if (birth && death) lifespan = `${birth} – ${death}`;
  else if (birth) lifespan = isDeceased ? `${birth} – ?` : `${birth}`;
  ...
  const content = (
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center gap-1 w-full px-0.5 sm:px-1 relative z-10">
        ...
        {lifespan && (
          <div className="text-[9px] sm:text-[10px] leading-none text-stone-500 tabular-nums whitespace-nowrap">
            {lifespan}
          </div>
        )}
      </div>
