...
// ── Helpers ──────────────────────────────────────────────────────────────────
/** Bỏ dấu tiếng Việt để tìm kiếm không dấu (kể cả đ → d). */
const deburr = (s: string) =>
  s
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "") // dấu thanh + dấu phụ đã tách ra khi NFD
    .replace(/[đĐ]/g, "d")
    .toLowerCase();
...
export default function PersonSelector({
  ...
  /** Đời tính động theo gốc đang chọn. Nếu không truyền, dùng person.generation. */
  ...
  const query = deburr(searchTerm.trim());
  const filteredPersons = persons
    .filter((p) => {
      const searchStr = deburr(`${p.full_name} ${p.birth_year || ""}`);
      return searchStr.includes(query);
    })
    // Sắp xếp theo năm sinh tăng dần (chưa rõ năm sinh xếp cuối), rồi theo tên.
    .sort((a, b) => {
          ...
          >
            {currentPerson ? (
              currentPerson.avatar_url ? (
                <Image
                  unoptimized
                  src={asset(currentPerson.avatar_url)}
              ...
              {filteredPersons.length > 0 ? (
                <div className="space-y-0.5">
                  {filteredPersons.map((person) => {
                    ...
                    const genValue = generationMap
                      ? generationMap.get(person.id) ?? null
                      : person.generation;
                    return (
                      <button
                        key={person.id}
